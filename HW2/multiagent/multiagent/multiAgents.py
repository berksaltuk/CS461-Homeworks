...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.maxi(gameState, 1, gameState.getNumAgents() - 1)
    
    def mini(self, gameState, depth, agentIndex, ghostNum):
        value = math.inf
        ...
        if agentIndex == ghostNum:
            if depth < self.depth:
                for successor in successors:
                    value = min(value, self.maxi(successor, depth+1, ghostNum))
            else:
                for successor in successors:
                    value = min(value, self.evaluationFunction(successor))
        else:
            for successor in successors:
                value = min(value, self.mini(successor, depth, agentIndex+1, ghostNum))
                    
        return value
    def maxi(self, gameState, depth, ghostNum):
        value = -math.inf
        ...
        for action in legalActions:
            ...
            val = self.mini(successor, depth, 1, ghostNum)
            if value < val:
                value = val
                best = action
        if depth > 1:
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState):
        ...
        for action in legalActions:
            ...
            score = self.mini(nextState, 0, 1, alpha, beta, ghostNum)
    ...
    def maxi(self, gameState, depth, alpha, beta, ghostNum):
        ...
        for action in legalActions:
            successor = gameState.generateSuccessor(0, action)
            value = max (value, self.mini(successor, depth+1, 1, alphaNew, beta, ghostNum))
    ...
    def mini(self, gameState, depth, agentIndex, alpha, beta, ghostNum):
            value = math.inf
            if gameState.isWin() or gameState.isLose():
                return self.evaluationFunction(gameState)
            ...
            for action in legalActions:
                ...
                if agentIndex == ghostNum:
                    value = min(value, self.maxi(successor, depth, alpha, betaNew, ghostNum))
                    
                    if value < alpha:
                        return value
                    betaNew = min(betaNew, value)
                else:
                    value = min(value, self.mini(successor, depth, agentIndex + 1, alpha, betaNew, ghostNum))
